...
class MapMaskGenerator:
    ...
    def free_vehicles_mask(self, vehicles: List[carla.Actor], free_veh_IDs) -> Mask:
        canvas = self.make_empty_mask()
        for veh in vehicles:
            if veh.attributes["role_name"] not in free_veh_IDs:
                continue
            bb = veh.bounding_box.extent
            corners = [
                carla.Location(x=-bb.x, y=-bb.y),
    ...
    # generate the mask for the arrows from the edge of the vehicle bounding box to the front of the vehicle
    def vehicle_arrows_mask(self, vehicles: List[carla.Actor]) -> Mask:
        canvas = self.make_empty_mask()
        # for the IROS version, the arrows are disabled!
        # for veh in vehicles:
        #     bb = veh.bounding_box.extent
        #     corners = [
        #         carla.Location(x=-bb.x, y=-bb.y),
        ...
        #     veh.get_transform().transform(corners)
        #     corners = [self.location_to_pixel(loc) for loc in corners]
        #     # get the front of the vehicle as the center of the corner 1 and corner 2
        #     front = np.mean([corners[1], corners[2]], axis=0)
        #     # get the end point of the arrow
        #     end_point = front + 1 * (front - np.mean(corners, axis=0))
        #     # transform the front and end point to integer
        #     front = (int(front[0]), int(front[1]))
        #     end_point = (int(end_point[0]), int(end_point[1]))
        #     # draw the arrow from the center of the bounding box to the front of the vehicle
        #     cv.arrowedLine(canvas, tuple(front), tuple(end_point), COLOR_ON, 2, tipLength=0.2)
        return canvas
    def occupied_vehicles_mask(self, vehicles: List[carla.Actor], occupied_veh_IDs) -> Mask:
        canvas = self.make_empty_mask()
        for veh in vehicles:
            if veh.attributes["role_name"] not in occupied_veh_IDs:
                continue
            bb = veh.bounding_box.extent
            corners = [
                carla.Location(x=-bb.x, y=-bb.y),
    ...
    def vehicle_numbers_mask(self, vehicles: List[carla.Actor]) -> Mask:
        canvas = self.make_empty_mask()
        for veh in vehicles:
            bb = veh.bounding_box.extent
            corners = [
            ...
            veh.get_transform().transform(corners)
            corners = [self.location_to_pixel(loc) for loc in corners]
            # add the last two digits of the vehicle id above the center of the vehicle bounding box
            center = np.mean(corners, axis=0)
            # the sizes of the text using roboto font
            text_size = cv.getTextSize(veh.attributes["role_name"], cv.FONT_HERSHEY_SIMPLEX, 0.5, 1)[0]
            half_text_width = text_size[0] // 2
            half_text_height = text_size[1] // 2
            # round to integer
            center = (int(center[0])-half_text_width, int(center[1])+half_text_height)
            # put the text on the canvas in the center of the bounding box
            cv.putText(canvas, veh.attributes["role_name"], center, cv.FONT_HERSHEY_SIMPLEX, 0.5, COLOR_ON, 2)
        return canvas

    def pedestrians_mask(self, pedestrians: List[carla.Actor], unassigned_ped_IDs=None) -> Mask:
        canvas = self.make_empty_mask()
        for ped in pedestrians:
            if not hasattr(ped, "bounding_box"):
                continue
            # check if the pedestrain is in the unassigned pedestrian list
            if ped.attributes["role_name"] not in unassigned_ped_IDs and unassigned_ped_IDs != None:
                continue
    ...
    def pedestrian_numbers_mask(self, pedestrians: List[carla.Actor], unassigned_ped_IDs=None) -> Mask:
        canvas = self.make_empty_mask()
        for ped in pedestrians:
            if not hasattr(ped, "bounding_box"):
                continue
            ...
            ped.get_transform().transform(corners)
            corners = [self.location_to_pixel(loc) for loc in corners]
            # add the last two digits of the pedestrian id above the center of the pedestrian bounding box
            center = np.mean(corners, axis=0)
            # the sizes of the text using roboto font
            text_size = cv.getTextSize(ped.attributes["role_name"], cv.FONT_HERSHEY_SIMPLEX, 0.5, 1)[0]
            half_text_width = text_size[0] // 2
            half_text_height = text_size[1] // 2
            # round to integer
            center = (int(center[0])-half_text_width, int(center[1])+half_text_height)
            # put the text on the canvas in the center of the bounding box
            cv.putText(canvas, ped.attributes["role_name"], center, cv.FONT_HERSHEY_SIMPLEX, 0.5, COLOR_ON, 2)
        return canvas
