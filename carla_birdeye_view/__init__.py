...
RGB_BY_MASK = {
    BirdViewMasks.VEH_ARROWS: RGB.BLACK,
    BirdViewMasks.PEDE_NUMBERS: RGB.WHITE,
    BirdViewMasks.VEH_NUMBERS: RGB.BLACK,
    ...
    BirdViewMasks.OCC_VEHS: RGB.ORANGE,
    BirdViewMasks.FREE_VEHS: RGB.GREEN,
...
class BirdViewProducer:
    ...
    def produce(
        self, predefined_loc=None, bev_type='schedule', free_vehicle_IDs=None, occupied_vehicle_IDs=None, unassigned_passenger_IDs=None
    ) -> BirdView:
        self.bev_type = bev_type
        if self.bev_type == 'schedule':
            self.free_vehicle_IDs = free_vehicle_IDs
            self.occupied_vehicle_IDs = occupied_vehicle_IDs
            if unassigned_passenger_IDs is not None:
                self.unassigned_passenger_IDs = unassigned_passenger_IDs
            else:
                self.unassigned_passenger_IDs = None
        all_actors = actors.query_all(world=self._world)
        segregated_actors = actors.segregate_by_type(actors=all_actors)
        if predefined_loc is None:
            raise NotImplementedError
        else:
            agent_vehicle_loc = carla.Location(x=predefined_loc[0], y=predefined_loc[1], z=0.1)
        ...
        masks = self._render_actors_masks(segregated_actors, masks) # render all the actors, modified by hcl, 2024.08

        cropped_masks = self.apply_agent_following_transformation_to_masks( masks,
        )
    ...
    def as_rgb(birdview: BirdView) -> RgbCanvas:
        ...
        # set the background color of the map to be green
        rgb_canvas[:,:,0] = 34
        rgb_canvas[:,:,1] = 139
        rgb_canvas[:,:,2] = 34
        # rgb_canvas[:,:,0] = 181
    ...
    def _render_actors_masks(
        ...
        """
        # MIDIFICATION: delete the light masks to avoid interference
        ...
        # )
        if self.bev_type == 'schedule':
            masks[BirdViewMasks.FREE_VEHS.value] = self.masks_generator.free_vehicles_mask(
                segregated_actors.vehicles, self.free_vehicle_IDs
            )
            masks[BirdViewMasks.OCC_VEHS.value] = self.masks_generator.occupied_vehicles_mask(
                segregated_actors.vehicles, self.occupied_vehicle_IDs
            )
            masks[BirdViewMasks.PEDESTRIANS.value] = self.masks_generator.pedestrians_mask(
                segregated_actors.pedestrians, self.unassigned_passenger_IDs
            )
            masks[BirdViewMasks.PEDE_NUMBERS.value] = self.masks_generator.pedestrian_numbers_mask(
                segregated_actors.pedestrians, self.unassigned_passenger_IDs
            )
        elif self.bev_type == 'graph':
            masks[BirdViewMasks.VEHICLES.value] = self.masks_generator.vehicles_mask(
                segregated_actors.vehicles
            )
        masks[BirdViewMasks.VEH_NUMBERS.value] = self.masks_generator.vehicle_numbers_mask(
            segregated_actors.vehicles
        )
        masks[BirdViewMasks.VEH_ARROWS.value] = self.masks_generator.vehicle_arrows_mask(
            segregated_actors.vehicles
        )
        return masks
